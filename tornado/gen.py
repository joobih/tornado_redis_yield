...
def async_redis_get(redis_cli, key, future, timeout, call_back=None, **kwargs):
    """
    author: 崔俊
    ...
    try:
        ...
        if not data:
            if timeout <= 0:
                IOLoop.current().call_later(0, lambda: call_back("", **kwargs))
            else:
                IOLoop.current().call_later(0.5, async_redis_get, redis_cli, key, future, timeout-1, call_back, **kwargs)
        else:
            IOLoop.current().call_later(0, lambda: call_back(data, **kwargs))
    except Exception as e:
        raise e


def redis_listen(redis_cli, key, call_back, timeout=10, **kwargs):
    timeout = timeout * 2
    future = Future()
    if call_back is not None:
        async_redis_get(redis_cli, key, future, timeout, call_back, **kwargs)
    return future
